...
// phases (migrate → `migrate:rollback --force`, force-confirmed because
// the adapter runs artisan non-interactively); irreversible phases — the
// cache phases and the queue restart signal, whose effects cannot be
// undone — return an informational success that does not block rollback
// (TS-P7-10 AC-2, TS-018-01-01 DoD).
//
...
// cacheFailureSemantics is the shared failure semantics of the
// cache-warming phases (config_cache, route_cache, event_cache): caches
// reflect exactly one release's code and are never repaired, only
// regenerated by re-running the activation.
const cacheFailureSemantics = "a failing cache phase fails the activation; " +
...
var phases = []phase{
	{
		...
		// --force skips Laravel's interactive confirmation prompt
		// (RollbackCommand uses ConfirmableTrait): the adapter runs
		// artisan as a non-interactive subprocess, where the default
		// confirmation answer is "no" — without --force the rollback
		// would always be cancelled in production (APP_ENV=production).
		rollbackArgs: []string{"migrate:rollback", "--force"},
	...
	},
	{
		...
		irreversible:     true,
		failureSemantics: cacheFailureSemantics,
	},
	{
