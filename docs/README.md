...
| **Verification** | 12 Laravel checks — 8 structural (files/structures must exist) + 4 lifecycle-conformity (shared-resource wiring, migration timing, queue restart, rollback behavior) — that must pass before an artifact is installed on a server and at the post-activation verify position ([verify.md](verify.md)) |
